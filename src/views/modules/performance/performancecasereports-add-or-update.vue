<template>
  <el-dialog
    ...
    <el-form-item label="避免跨系统编码错误，实际文件存储的报告名，名称和测试报告文件夹名称一致" prop="reportName">
      <el-input v-model="dataForm.reportName" placeholder="避免跨系统编码错误，实际文件存储的报告名，名称和测试报告文件夹名称一致"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          reportName: [
            { required: true, message: '避免跨系统编码错误，实际文件存储的报告名，名称和测试报告文件夹名称一致不能为空', trigger: 'blur' }
          ],
          fileSize: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.reportId) {
            this.$http({
              url: this.$http.adornUrl(`/performance/performancecasereports/info/${this.dataForm.reportId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.caseId = data.performanceCaseReports.caseId
                this.dataForm.fileId = data.performanceCaseReports.fileId
                this.dataForm.originName = data.performanceCaseReports.originName
                this.dataForm.reportName = data.performanceCaseReports.reportName
                this.dataForm.fileSize = data.performanceCaseReports.fileSize
                this.dataForm.status = data.performanceCaseReports.status
                this.dataForm.remark = data.performanceCaseReports.remark
                this.dataForm.addTime = data.performanceCaseReports.addTime
                this.dataForm.addBy = data.performanceCaseReports.addBy
                this.dataForm.updateTime = data.performanceCaseReports.updateTime
                this.dataForm.updateBy = data.performanceCaseReports.updateBy
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/performance/performancecasereports/${!this.dataForm.reportId ? 'save' : 'update'}`),
              method: 'post',
