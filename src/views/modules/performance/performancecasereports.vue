  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('performance:performancecasereports:save')" type="primary" @click="addOrUpdateHandle()">修改</el-button>
        <el-button v-if="isAuth('performance:performancecasereports:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">删除</el-button>
        <el-button v-if="isAuth('performance:performancecasereports:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">仅删除结果文件</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
...
<script>
  ...
  import {getPerTestCase, getTestRepList} from '../../../api/api'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        let params = {page: this.pageIndex, limit: this.pageSize, key: this.dataForm.key};
        let headers = {token: this.$cookie.get('token')};
        getTestRepList(headers, params).then((data) => {
          if (data && data.code === 0) {
