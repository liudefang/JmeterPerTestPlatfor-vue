    ...
    colModel: [
      ...
      {label: '用例名称',
        ...
        formatter: function (value, options, row) {
          ...
          if (row.debugStatus == 1) { return "<a href='" + baseURL + 'modules/test/debugTestReports.html?CaseID=' + row.caseId + "' title='" + value + " 调试报告'>" + value + '</a>' } else { return "<a href='" + baseURL + 'modules/test/stressTestReports.html?CaseID=' + row.caseId + "' title='" + value + " 测试报告'>" + value + '</a>' }
        }
      },
      {
        label: '文件名称',
...
var vm = new Vue({
  ...
  methods: {
    ...
    chkCronEditType: function () {
      if (vm.stressTestFile.cronEditType == 1) {
